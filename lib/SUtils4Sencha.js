...
SUtils4Sencha = {
	 ...
	 * 
	 */
	startRecording : function(captureSuccess, captureError){
		
		SUtils.log("startRecording");
		
		if(navigator.device && navigator.device.capture){
			
			var plugin = navigator.device.capture;
			
			// start video capture
			plugin.captureVideo(captureSuccess, captureError, {});
		}else{
