...
SUtils4Sencha = {
	...
	/**
	 * Hace una petición para solicitar el registro depl push
	 *
	 * @method registerPushService
	 * @param {function} successCallback
	 * @param {functoin} errorCallback
	 * @param {object} options
	 */
	registerPushService :function(options,callback){
		...
		if(window.plugins && window.plugins.pushNotification ){
			...
			function(error){
				callback && callback(null, error.message ? error.message : error);
			},
			options);
		}else{
			callback && callback(null, "Push Plugin not found");
		}
	},
    	...
    	if(navigator.notification){
    		if(buttons){
    			navigator.notification.confirm(
    					...
    					title || SGlobalize.translate("confirm"),
    					buttons
    			);
    		}else{
    			navigator.notification.confirm(
    					...
    					SGlobalize.translate("confirm"),
    					SGlobalize.translate("aceptar")+','+
    					SGlobalize.translate("cancelar")
    			);
    		}
    	}else if( confirm(msg)  ){
